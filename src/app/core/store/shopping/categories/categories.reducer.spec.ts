...
describe('Categories Reducer', () => {
  ...
  describe('LoadCategory actions', () => {
    describe('LoadCategory action', () => {
      it('should set loading to true', () => {
        const action = loadCategory({ payload: { categoryId: '123' } });
    ...
    describe('LoadCategoryFail action', () => {
      it('should set loading to false', () => {
        const action = loadCategoryFail({ payload: { error: {} as HttpError } });
        const state = categoriesReducer(initialState, action);
    ...
    describe('LoadCategorySuccess action', () => {
      ...
      it('should insert category if not exists', () => {
        const action = loadCategorySuccess({ payload: { categories: categoryTree([category]) } });
        const state = categoriesReducer(initialState, action);
      ...
      it('should update category if already exists', () => {
        const action1 = loadCategorySuccess({ payload: { categories: categoryTree([category]) } });
        ...
        const action2 = loadCategorySuccess({ payload: { categories: categoryTree([updatedCategory]) } });
  ...
  describe('LoadTopLevelCategoriesSuccess action', () => {
    ...
    it('should add all flattened categories to the entities state', () => {
      const action = loadTopLevelCategoriesSuccess({ payload: { categories } });
      const state = categoriesReducer(initialState, action);
