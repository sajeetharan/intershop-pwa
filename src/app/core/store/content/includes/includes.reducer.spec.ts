...
describe('Includes Reducer', () => {
  ...
  it('should set loading state when reducing LoadContentInclude', () => {
    const newState = includesReducer(initialState, loadContentInclude({ payload: { includeId: '' } }));
  ...
  describe('after begin loading', () => {
    ...
    beforeEach(() => {
      loadingState = includesReducer(initialState, loadContentInclude({ payload: { includeId: '' } }));
    });

    it('should unset loading state when reducing LoadContentIncludeFail', () => {
      const newState = includesReducer(
        loadingState,
        loadContentIncludeFail({ payload: { error: { message: 'ERROR' } as HttpError } })
      );
